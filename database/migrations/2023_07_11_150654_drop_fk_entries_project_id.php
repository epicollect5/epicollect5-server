...
class DropFkEntriesProjectId extends Migration
{
    ...
    public function down()
    {
        ...
        Schema::table('entries', function (Blueprint $table) {
            $table->foreign('project_id', 'fk_entries_project_id')->references('id')->on('projects');
        });

        Schema::table('entries_archive', function (Blueprint $table) {
            $table->foreign('project_id', 'fk_entries_archive_project_id')->references('id')->on('projects');
        });

        Schema::table('branch_entries_archive', function (Blueprint $table) {
            $table->foreign('project_id', 'fk_branch_entries_archive_project_id')->references('id')->on('projects');
        });
    }
}
