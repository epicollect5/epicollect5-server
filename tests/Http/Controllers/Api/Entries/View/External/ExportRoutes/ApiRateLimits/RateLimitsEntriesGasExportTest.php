...
namespace Http\Controllers\Api\Entries\View\External\ExportRoutes\ApiRateLimits;
 ...
 */
class RateLimitsEntriesGasExportTest extends TestCase
{
    use Assertions;
    ...
    public function setUp(): void
    {
        ...
        $name = config('testing.API_RATE_LIMITS_ENTRIES_GAS.name');
        $this->slug = config('testing.API_RATE_LIMITS_ENTRIES_GAS.slug');
     ...
     * @throws Throwable
     */
    public function test_rate_limit_api_entries_google_apps_scripts()
    {
        $apiEntriesRateLimit = config('epicollect.limits.api_export.google_apps_scripts');
        ...
        $entriesClient = new Client([
            'headers' => [
                'User-Agent' => 'Mozilla/5.0 (compatible; Google-Apps-Script; beanserver; +https://script.google.com; id: UAEmdDdTest)',
        ...
        // travel() expects seconds, so we travel forward 100 minutes (600 seconds)
        $this->travel(6000);
