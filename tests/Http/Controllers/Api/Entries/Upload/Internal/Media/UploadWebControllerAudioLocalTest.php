...
class UploadWebControllerAudioLocalTest extends TestCase
{
    ...
    public function test_it_should_upload_a_top_hierarchy_audio()
    {
        $response = [];
        $inputRef = null;
        try {
            ...
            $audioFilename = $entryPayloads[0]['data']['entry']['answers'][$inputRef]['answer'];
            ...
            //get compressed audio file size
    ...
    public function test_it_should_upload_a_top_hierarchy_entry_with_multiple_files()
    {
        ...
        try {
            ...
            //get compressed audio and video file size
    ...
    public function test_it_should_upload_a_child_entry_audio()
    {
        $response = [];
        $inputRef = null;
        ...
        try {
            ...
            //Get an audio file and save it to temp disk
            $audioFilename = $childEntryPayloads[0]['data']['entry']['answers'][$inputRef]['answer'];
            $audioPath = '/audio/'. $this->project->ref . '/' . $audioFilename;
            //get the test audio file
            $sampleAudioFilePath = base_path('tests/Files/audio.mp4');
            // Get the size before copying
     ...
     * @throws Throwable
     */
    public function test_it_should_upload_a_branch_audio()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        $branchInputs = [];
        ...
        $audioPath = '/audio/'. $this->project->ref . '/' . $filename;
        //get the test audio file
        $sampleAudioFilePath = base_path('tests/Files/audio.mp4');
        // Get the size before copying
        $audioFileSize = filesize($sampleAudioFilePath);
