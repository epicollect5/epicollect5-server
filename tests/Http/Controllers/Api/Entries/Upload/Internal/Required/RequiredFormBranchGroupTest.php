...
class RequiredFormBranchGroupTest extends TestCase
{
    ...
    public function test_form_required_branch_group_text_by_web_upload()
    {
        //get branch inputs
                    ...
                    if ($branchInput['type'] === config('epicollect.strings.inputs_type.group')) {
                        ...
                        foreach ($groupInputs as $groupInputIndex => $groupInput) {
                            if ($groupInput['type'] === config('epicollect.strings.inputs_type.text')) {
                                $this->projectDefinition['data']['project']['forms'][0]['inputs'][$index]['branch'][$branchInputIndex]['group'][$groupInputIndex]['is_required'] = true;
    ...
    public function test_form_required_branch_group_integer_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_decimal_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_phone_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_date_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_time_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_radio_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_dropdown_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_checkbox_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_textbox_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
    ...
    public function test_form_required_branch_group_searchsingle_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
            ...
            if ($input['type'] === config('epicollect.strings.inputs_type.branch')) {
                ...
                foreach ($branchInputs as $branchInputIndex => $branchInput) {
                    if ($branchInput['type'] === config('epicollect.strings.inputs_type.group')) {
                        ...
                        $searchsingleInput = ProjectDefinitionGenerator::createSearchSingleInput($branchInput['ref']);
                        $searchsingleInput['is_required'] = true;
                        $this->projectDefinition['data']['project']['forms'][0]['inputs'][$index]['branch'][$branchInputIndex]['group'][] = $searchsingleInput;
                        $branchGroupInputRef = $searchsingleInput['ref'];
                        //override entry generator with new project definition
                        $this->entryGenerator = new EntryGenerator($this->projectDefinition);
                        //add group input to the branch inputs
                        $branchInputs[$branchInputIndex]['group'][] = $searchsingleInput;
                        break 2;
                    }
                }
            }
    ...
    public function test_form_required_branch_group_searchmultiple_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
            ...
            if ($input['type'] === config('epicollect.strings.inputs_type.branch')) {
                ...
                foreach ($branchInputs as $branchInputIndex => $branchInput) {
                    if ($branchInput['type'] === config('epicollect.strings.inputs_type.group')) {
                        ...
                        $searchmultipleInput = ProjectDefinitionGenerator::createSearchMultipleInput($branchInput['ref']);
                        $searchmultipleInput['is_required'] = true;
                        $this->projectDefinition['data']['project']['forms'][0]['inputs'][$index]['branch'][$branchInputIndex]['group'][] = $searchmultipleInput;
                        $branchGroupInputRef = $searchmultipleInput['ref'];
                        //override entry generator with new project definition
                        $this->entryGenerator = new EntryGenerator($this->projectDefinition);
                        //add group input to the branch inputs
                        $branchInputs[$branchInputIndex]['group'][] = $searchmultipleInput;
                        break 2;
                    }
                }
            }
    ...
    public function test_form_required_branch_group_barcode_by_web_upload()
    {
        //get branch inputs
        $inputs = array_get($this->projectDefinition, 'data.project.forms.0.inputs');
        //get the first text question of the first group in the branch and add form uniqueness
     ...
     */
    private function setRequiredAnswerAsEmpty($payloadAnswers, &$payload, $inputRef, $inputAnswer)
    {
