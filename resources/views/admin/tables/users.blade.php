...
@else
    <div class="row">
    <div class="col-xs-4 pull-right user-list__total text-right text-capitalize">
    ...
    <table class="table table-bordered table-hover table-condensed user-administration__table">
        ...
        @foreach ($users as $user)
            <tr>
                <td>@if(!empty($user->name)){{ $user->name . ' ' . $user->last_name }} @else <i>n/a</i> @endif</td>
                <td>{{ $user->email }}</td>
                ...
                <td class="text-center">
                    <form method="POST" action="{{ url('/admin/update-user-server-role') }}" accept-charset="UTF-8"
                        ...
                        @elseif ($user->server_role === 'basic')
                            ...
                            @endif
                        @else <span>{{trans('site.super_admin')}}</span>
                        @endif
                    </form>
                </td>
            </tr>
