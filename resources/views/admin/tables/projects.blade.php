<div class="projects__table__wrapper">
    @if (count($projects) == 0) <p class="well">{{ trans('site.no_projects_found')}}</p>
    @else
        <div class="col-xs-4 pull-right projects-list__total text-right text-capitalize">
            <span>Total: {{$projectsTotal}}</span>
        </div>
        <table class="table table-bordered table-hover table-condensed projects__table">
            ...
            @foreach ($projects as $project)
                <tr>
                    <td class="text-center"><img class="project-logo"
                             width="32"
                             height="32"
                        ...
                        <div class="btn-group">
                            <select class="form-control project-roles" data-project-id="{{ $project->project_id }}">
                                ...
                                @foreach (Config::get('ec5Enums.project_roles') as $role)
                                    <option value="{{$role}}"
                                            @if($project->my_role == $role) selected @endif>{{trans('site.project_roles.'.$role)}}
                                    </option>
                                @endforeach
                            </select>
                        </div>
