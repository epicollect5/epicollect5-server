<div>
    @if($project->access === 'private')
        <div class="warning-well">Endponts are not accessible by a browser for <strong>private</strong> projects</div>
    @endif
        ...
        <tr>
            ...
            <td>
                @if($project->access === 'public')
                    <a href="https://five.epicollect.net/api/export/project/{{$project->slug}}"
                       target="_blank"
                    >
                        {{url('/')}}/api/export/project/{{$project->slug}}
                    </a>

                @else
                    <span>{{url('/')}}/api/export/project/{{$project->slug}}</span>
                @endif
            </td>
        </tr>
